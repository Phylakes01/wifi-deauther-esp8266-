...
void DisplayUI::setup() {
    ...
    createMenu(&mainMenu, NULL, [this]() {
        ...
        addMenuNode(&mainMenu, D_CLOCK, &clockMenu); // GAMES
    ...
    createMenu(&clockMenu, &mainMenu, [this]() {
        ...
        // Add a menu entry to launch the eyes/emoji mode.  Selecting
        // this item sets the global eyesMode flag defined in Eyes.cpp.
        // The main loop in ESP8266_Deauther_SSD1306.ino will detect
        // this flag and call eyesLoop() until the user exits eyes mode.
        addMenuNode(&clockMenu, "EYES / EMOJI", [this]() {
            eyesMode = true;
        });

        addMenuNode(&clockMenu, "DINO GAME", [this]() { // DINO GAME
            resetDinoGame();
            mode = DISPLAY_MODE::DINO_GAME;
            display.setFont(DejaVu_Sans_Mono_12);
...
void DisplayUI::setupButtons() {
    ...
    up->setOnClicked([this]() {
        ...
        if (!tempOff) {
            ...
            } else if (mode == DISPLAY_MODE::DINO_GAME) {     // D5 / UP = JUMP only
    ...
    down->setOnClicked([this]() {
        ...
        if (!tempOff) {
            if (mode == DISPLAY_MODE::MENU) {                 // when in menu, go up or down with cursor
                ...
                setTime(clockHour, clockMinute - 1, clockSecond);
            } else if (mode == DISPLAY_MODE::DINO_GAME) {     // D6 / DOWN = RESET game
                resetDinoGame();
            }
        }
    });
    ...
    a->setOnClicked([this]() {
        ...
        if (!tempOff) {
            switch (mode) {
                ...
                case DISPLAY_MODE::DINO_GAME:
                    // D7 / SELECT = EXIT back to menu
                    mode = DISPLAY_MODE::MENU;
                    display.setFont(DejaVu_Sans_Mono_12);
                    display.setTextAlignment(TEXT_ALIGN_LEFT);
                    break;
...
void DisplayUI::resetDinoGame() {
    ...
    dinoJumping   = false;
    dinoGameOver  = false;
    dinoMoveTime  = currentTime;
}

void DisplayUI::updateDinoGame() {
    // DIRECT BUTTON READ only for D5 jump and D6 reset.
    // D7 SELECT is handled by normal Button click to avoid glitches.
    // Buttons use INPUT_PULLUP/ButtonPullup, so pressed = LOW.
    ...
    // D5 / UP = instant jump on press edge
    ...
    // D7 / SELECT is NOT direct-read here.
    // It stays on the normal Button click handler to avoid menu/back glitches.
    ...
    // Game physics still uses frame interval, but button input above is real-time.
    if (currentTime - dinoMoveTime < 30) return;
    dinoMoveTime = currentTime;
    ...
    // Collision: dino box x 12..24, y dinoY..54; cactus box x cactusX..cactusX+7, y 42..54
    if ((cactusX < 24) && (cactusX + 7 > 12) && (dinoY + 10 > 42)) {
        dinoGameOver = true;
    }
}

void DisplayUI::drawDinoGame() {
    updateDinoGame();
    ...
    // Ground
    drawLine(0, 55, 127, 55);

    // Score
    drawString(0, 0, "SCORE:" + String(dinoScore));

    // Dino
    display.fillRect(12, dinoY, 10, 10);
    display.fillRect(20, dinoY + 6, 4, 4);

    // Dino legs
    display.fillRect(13, dinoY + 10, 3, 3);
    display.fillRect(19, dinoY + 10, 3, 3);

    // Cactus
    display.fillRect(cactusX, 42, 6, 13);
    display.fillRect(cactusX - 3, 46, 3, 4);
    display.fillRect(cactusX + 6, 47, 3, 4);

    // Show only GAME OVER when player loses
    if (dinoGameOver) {
        display.setTextAlignment(TEXT_ALIGN_CENTER);
