...
#include "CLI.h"
#include "DisplayUI.h"
#include "Eyes.h" // Pull in the eyes/emoji subsystem
#include "A_config.h"
...
void setup() {
    ...
    if (settings::getDisplaySettings().enabled) {
        ...
        // Initialise the eyes animation subsystem.  This call sets up
        // a secondary SSD1306 controller and prepares the RoboEyes
        // animations.  Without this, selecting the "EYES / EMOJI"
        // menu entry will have no visible effect.
        eyesSetup();
    }
...
void loop() {
    // When eyesMode is active, run the eyes animation handler.  If
    // eyesLoop() sets eyesMode to false (i.e. the user exited
    // eyes mode), fall through to the normal tasks so that the
    // display immediately refreshes to the menu.  Otherwise, return
    // early to suspend the deauther tasks during the eyes animation.
    if (eyesMode) {
        eyesLoop();
        // If eyesMode is still true after the update, remain in
        // eyes mode and skip the rest of the loop.  If it changed
        // to false (exit pressed), continue with the normal UI
